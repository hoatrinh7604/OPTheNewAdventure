	...
	void Update () 
	{
		...
		if(Input.GetMouseButtonUp(0))
		{
			for (int i = 0; i < levelList.Count; i++)
			{
				...
				{
					currentLevelIndex = levelList [i].currentLevelIndexMap;//Me: del
					SceneManager.LoadScene(levelList[i].LevelIndex);//Me: SceneManager.LoadScene(levelList [i].LevelIndex);
...
public class LevelList
{
	...
	public int LevelIndex;						//Level index, can be found in the build settings; => to CharacterScene
	public int currentLevelIndexMap;				//Get Current level selected
